    ...
    for param in params:
        if module.params[param]:
            if module.argument_spec[param].get("type", "str") in ["str", "int", "float", "path", "raw", "bytes"]:
                args.append(f"{params[param]}={str(module.params[param])}")
                ...
                for item in module.params[param]:
                    args.append(f"{params[param]}={str(item)}")

    if not module.check_mode:
        rc, result["stdout"], result["stderr"] = module.run_command(args)
        if rc != 0:
            if ("error allocating terminal" in result["stderr"] or
                    "open /dev/tty: no such device or address" in result["stderr"]):
                result["msg"] = (
                ...
                result["msg"] = f"Error running command \'{' '.join(args)}\'"
